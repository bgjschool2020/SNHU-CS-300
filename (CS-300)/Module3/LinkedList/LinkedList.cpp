...
void LinkedList::Remove(string bidId) {
    ...
    Node* oldHead = head;  // create node for old head to be deleted

    // wwon't delete anything if the list is empty
    if (oldHead == nullptr) {
        return;
    }

    // special case if matching node is the head
    if (bidId == oldHead->bid.bidId) {
    ...
    // start at the head
    Node* TBD = head;  // create node To Be Deleted
    // while loop over each node looking for a match
    while (TBD -> next) {
        // if the next node bidID is equal to the current bidID
        if (bidId == TBD->next->bid.bidId) {
